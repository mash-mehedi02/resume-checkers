 ...
 * Implements skill matching, experience scoring, education scoring, project
 * scoring,
 * and final weighted score calculation.
 * 
 * BIAS CONTROL MEASURES:
 * =====================
 * 1. SKILL-FIRST SCORING: Skills are weighted at 50% (highest weight), ensuring
 * that
 * technical competence is the primary factor in candidate evaluation. This
 * prevents
 * bias based on personal characteristics.
 * 
 * 2. NO NAME/GENDER USAGE: Candidate names are NOT used in scoring or ranking
 * algorithms.
 * The candidateName field is optional and stored only for display purposes. All
 * scoring
 * is based purely on technical qualifications (skills, experience, education,
 * projects).
 * 
 * 3. CONFIGURABLE WEIGHTS: Scoring weights are configurable via
 * application.yml, allowing
 * organizations to adjust the importance of different factors based on their
 * needs while
 ...
 * - Photo/images
 * 
 * These measures ensure fair, unbiased candidate evaluation based solely on
 * qualifications.
 */
@Service
public class ScoringService {
    ...
    // BIAS CONTROL: Configurable weights allow organizations to adjust scoring
    // while
    // maintaining skill-first approach. Skills are always weighted highest (60%
    // default)
    // to ensure technical competence is the primary evaluation factor.
    @Value("${app.scoring.skill-weight:0.60}")
    private double skillWeight;

    @Value("${app.scoring.experience-weight:0.20}")
    private double experienceWeight;

    @Value("${app.scoring.education-weight:0.05}")
    private double educationWeight;

    @Value("${app.scoring.project-weight:0.15}")
     ...
     * (Project Score × 0.10)
     * 
     * BIAS CONTROL: Skills are weighted at 50% (highest), ensuring skill-first
     * evaluation.
     * This means technical competence is the primary factor, preventing bias based
     * on
